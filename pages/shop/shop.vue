<template>
	<view class="container">
		<!-- 头部 -->
		<view class="tui-shop__info" style="height:120px;">
			<image src="../../static/images/follow/2.jpg" class="tui-logo" @tap="shop"></image>
		...
		<!-- Tabs页 -->
		<tui-tabs @change="change" :currentTab="current" :tabs="tabs" itemWidth="25%" backgroundColor="#18293d"
		...
		<view class="tui-items__1 tui-padding" v-if="current == 0">
			...
			<block v-for="(model, key) in rankingList" :key="key">
				<view class="tui-ranking__list" :class="{ 'tui-justify__start': model.list.length === 2 }"
					v-if="rankingTab === key">
					<view class="tui-ranking__item" @tap="detail"
						:class="{ 'tui-item-mr__16': model.list.length === 2 }" v-for="(item, index) in model.list"
						:key="index">
						<image :src="item.img"></image>
						<view class="tui-ranking__gtitle">{{ item.title }}</view>
						<view class="tui-ranking__sub">
							{{ key == 0 ? item.sales : item.collection }}人{{ key == 0 ? '购买' : '收藏' }}
						</view>
					</view>
				</view>
			</block>
			...
			<view class="tui-recommend__item" @tap="detail" v-for="(item, index) in 12" :key="index">
				...
				<view class="tui-recommend__right">
					...
					<view class="tui-rg__interested">
						<tui-icon name="like" :size="26" unit="rpx" color="#fa436a"></tui-icon>
		...
		<view class="tui-items__2" v-if="current == 1">
			<view class="tui-header__screen">
				<view class="tui-screen__item" :class="{ 'tui-screen__active': screenType == 1 }" @tap="screen(1)">综合
				</view>
				<view class="tui-screen__item" :class="{ 'tui-screen__active': screenType == 2 }" @tap="screen(2)">销量
				</view>
				<view class="tui-screen__item" :class="{ 'tui-screen__active': screenType == 3 }" @tap="screen(3)">
					...
					<view class="tui-screen__price">
						<view class="tui-price__asc">
							<tui-icon name="turningup" :color="screenType == 3 && sortWay == 'asc' ? '#E32424' : '#000'"
								:size="24" unit="rpx"></tui-icon>
						</view>
						<view class="tui-price__desc">
							<tui-icon name="turningdown"
								:color="screenType == 3 && sortWay == 'desc' ? '#E32424' : '#000'" :size="24"
								unit="rpx"></tui-icon>
						</view>
					</view>
				</view>
				<view class="tui-screen__item" @tap="screen(4)">
					<tui-icon :name="isList ? 'manage' : 'listview'" :size="isList ? 44 : 34" unit="rpx"
		...
		<!--=======分类 start=======-->
		<view class="tui-items__5 tui-padding" v-if="current == 3">
			<view class="tui-classify-box" @tap="product">
				<tui-list-cell padding="30rpx" :hover="false" arrow unlined>
					<view class="tui-classify__main">加菲猫</view>
				</tui-list-cell>
			...
			<view class="tui-classify-box" @tap="product">
				<tui-list-cell padding="30rpx" :hover="false" arrow unlined>
					<view class="tui-classify__main">英国短毛猫</view>
				</tui-list-cell>
			</view>
			<view class="tui-classify-box" @tap="product">
				<tui-list-cell padding="30rpx" :hover="false" arrow unlined>
					<view class="tui-classify__main">德文</view>
				</tui-list-cell>
			</view>
			<view class="tui-classify-box" @tap="product" v-for="(item, index) in 3" :key="index">
				<tui-list-cell padding="30rpx" :hover="false" arrow unlined>
					<view class="tui-classify__main">矮脚猫</view>
				</tui-list-cell>
				<view class="tui-classify__sub">
				...
				if (index == 4) {
					...
					if (index == 1) {
						...
						this.productList.sort(function(a, b) {
							let differ;
							if (self.sortWay == 'asc') {
								differ = a.sale - b.sale;
							} else {
								differ = b.sale - a.sale;
							}
							return differ;
						})
					}
