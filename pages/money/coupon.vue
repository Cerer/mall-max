...
<template>
	<view class="container">
		<tui-tabs :tabs="tabs" :currentTab="currentTab" selectedColor="#fa436a" sliderBgColor="#fa436a" @change="change"
		...
		<view class="tui-coupon-list" :class="{'tui-mtop':scrollTop>=0}">
			<view v-for="(item,i) in couponList" :key="i" v-show="currentTab==i">
				...
				<view class="tui-coupon-item tui-top20" v-for="(coupon,j) in item.data" :key="j">
					<image src="../../static/images/coupon/bg_coupon_3x.png" class="tui-coupon-bg" mode="widthFix">
					</image>
					<image :src="getSignUrl(i)" class="tui-coupon-sign" v-if="i>0"></image>
					...
					<view class="tui-coupon-item-left">
						<view class="tui-coupon-price-box" :class="{'tui-color-grey':i>0}">
							...
							<view class="tui-coupon-price"
								:class="{'tui-price-small':coupon.value.toString().length>4}">{{coupon.value}}</view>
						...
						<view class="tui-coupon-content">
							...
							<view class="tui-coupon-rule">
								...
								<view class="tui-rule-box">
									<view class="tui-coupon-circle"></view>
									<view class="tui-coupon-text">{{j%2==0?"自领取之日起30天有效":"2019-11-01至2019-12-31"}}
									</view>
								</view>
							</view>
						</view>
					...
					<navigator hover-class="none" url="../../index/productDetail/productDetail" class="tui-btn-box"
						v-if="i===0">
						<tui-form-button background="#fa436a" color="#fa436a" width="152rpx" height="52rpx"
							radius="50rpx" :size="24" plain @click="clickUse">
							立即使用
						</tui-form-button>
					</navigator>
	...
	export default {
		...
		methods: {
			getSignUrl: function(index) {
				let url = "";
				if (index == 1) {
					url = "../../static/images/coupon/img_coupon_beused_3x.png"
				} else if (index == 2) {
					url = "../../static/images/coupon/img_coupon_failure_3x.png"
				}
				return url
			},
